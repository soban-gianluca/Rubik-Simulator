...
class RubiksCube:
    ...
    def get_cube_color(self, x, y, z, face_index):
        ...
        if face_index == 0 and y == 1:  # Top face
            row, col = 1-z, x+1
        elif face_index == 1 and y == -1:  # Bottom face
            row, col = z+1, x+1
        elif face_index == 2 and x == 1:  # Right face
            row, col = 1-y, z+1
        elif face_index == 3 and x == -1:  # Left face
            row, col = 1-y, 1-z
        elif face_index == 4 and z == 1:  # Front face
            row, col = 1-y, x+1
        elif face_index == 5 and z == -1:  # Back face
            row, col = 1-y, 1-x
        else:
    ...
    def move_R(self, clockwise=True):
        ...
        if clockwise:
            ...
            # Move edge pieces (reverse of clockwise)
            temp = self.faces['top'][:, 2].copy()
            self.faces['top'][:, 2] = self.faces['back'][:, 0][::-1]
            self.faces['back'][:, 0] = self.faces['bottom'][:, 2][::-1]
    ...
    def move_L(self, clockwise=True):
        ...
        if clockwise:
            ...
            self.faces['bottom'][:, 0] = self.faces['back'][:, 2][::-1]
            self.faces['back'][:, 2] = temp[::-1]
    
    def move_U(self, clockwise=True):
        """Rotate the upper (top) face"""
    ...
    def move_F(self, clockwise=True):
        ...
        if clockwise:
            ...
            temp = self.faces['top'][2, :].copy()
            self.faces['top'][2, :] = self.faces['left'][:, 2][::-1]
            self.faces['left'][:, 2] = self.faces['bottom'][0, :]
            self.faces['bottom'][0, :] = self.faces['right'][:, 0][::-1]
            self.faces['right'][:, 0] = temp
        else:
